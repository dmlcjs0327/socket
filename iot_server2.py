...
water_moter_err = False #수위모터에 에러가 있는지 (0기본, 1에러)
cooler_moter_err = False #스프링쿨러에 에러가 있는지 (0기본, 1에러)
...
thread_sound_send_is_run = False
...
def state_message():
    ...
    -사이렌 정보: {}
    -모터 정보: {}
    """.format(humity_target, humity_real, capture_option, siren_option, motor_option)
    append_message(msg)
...
def _connect():
    ...
    if not is_end and socket_is_connected:
        print("[_connect] 소켓이 이미 연결되어 있습니다")
        print("[_connect] <종료>")
        return
    
    while not is_end:
        try:
            print("[_connect] 연결 시도 중...")
            client_socket, client_addr = server_socket.accept() #연결될 때까지 대기
            print("[_connect] 연결 성공 with {}".format(client_addr))
            socket_is_connected = True
            err_cnt = 0
            break
            
        except Exception as e:
            print("[_connect] 에러: {}".format(e))
            sleep(interval) #interval sec만큼 대기
            err_cnt += 1
            if err_cnt > 0:                
                print("[_connect] 에러가 {}초 이상 수정되지 않았습니다. 연결을 중단합니다.".format(interval*err_cnt))   
                connect_err = True
                break
...
def _receive():
    ...
    while not is_end:
        try: 
            ...
            print("[_receive] 수신: {}".format(recv_data))
            ...
            tmp.append(n)
            tmp = tmp[2:]
            print("[_receive] 변환 data: {}".format(tmp))
            ...
            if siren_option == 1 and not thread_sound_send_is_run: #사이렌이 꺼져있으면 실행
                thread_sound_send_is_run = True
                thread_sound_send.run()
...
def _sound_send():
    global is_end
    global thread_sound_send_is_run
...
def _sensor():
    ...
    water_bottom_values = [] #저수탱크 하단 센서값을 저장할 리스트
    water_top_values = [] #저수탱크 상단 센서값을 저장할 리스트
    humity_values = [] #저수탱크 상단 센서값을 저장할 리스트
    ...
    while not is_end:
        ...
        try:
            ...
            water_bottom_values.append(water_level_bottom)
            water_top_values.append(water_level_top)
            
            if len(water_bottom_values) == 10: #센서값을 10개 모은 경우에 수행
                water_level_bottom = 1 if avg(water_bottom_values)>sensor_thres else 0 #평균값 추출
                water_level_top = 1 if avg(water_top_values)>sensor_thres else 0 #평균값 추출
                
                water_bottom_values = [] #리스트 초기화
                water_top_values = [] #리스트 초기화
                            
                if water_level_bottom == 0: #저수탱크 하단에 물이 없으면
                    if not water_moter_on:#모터가 꺼져있으면
                        pump(1, 0, 1, ENA_T, IN1_T, IN2_T) #모터 작동
                        append_message("[_sensor] 저수탱크 모터 작동")
                    else: #모터가 켜져있으면
                        water_level_bottom_cnt+=1 #물이 없는 상태 카운트
                        if water_level_bottom_cnt > 20: raise #20회 초과 시 에러처리
        ...
        try:
            ...
            humity = 64-int(adcValue/1023) #수정해야함!!!!!!!!!!!!!!!!!!!!!!
            
            humity_values.append(humity)
            
            if len(humity_values) == 10: #센서값을 10개 모은 경우에 수행
                humity_real = avg(humity_values)
                humity_values = [] #리스트 초기화
                ...
                if motor_option == 1:
                    ...
                    if humity_real < humity_target: #습도가 목표치보다 낮으면
                        if not cooler_moter_on: #모터가 꺼져있으면
                            ...
                            if humity_low_cnt > 20: raise #20회 초과 시 에러처리
                            
                    else : #그 외엔
                        humity_low_cnt = 0 #카운트 초기화
                        if cooler_moter_on: #모터가 켜져있으면
